    ...
    <section id="faqs" className="bg-[#f3f3f3] py-24 px-4 sm:px-6 lg:px-10">
      <div className="mx-auto w-full max-w-[1320px]">
        <div className="mb-12 rounded-[26px] px-0 py-0 sm:px-4">
          ...
          <h2 className="text-center text-[40px] md:text-[48px] font-black leading-tight text-[#161626]">
            Still Have <span className="text-[#ff5a14]">Questions?</span>
          </h2>
          <p className="mt-3 text-center text-[18px] md:text-[20px] text-[#6a6a6a]">
            Everything you need to know before ordering
          </p>
        </div>

        <div className="mx-auto mb-14 w-full max-w-[1150px] space-y-3">
          {faqs.map((item) => {
            const isOpen = openId === item.id;
            return (
              <article
                key={item.id}
                className={`overflow-hidden rounded-[12px] ${isOpen ? "bg-[#ff5a14]" : "bg-[#ececec]"
                  }`}
              >
                <button
                ...
                >
                  <h3
                    className={`pr-4 text-[18px] md:text-[22px] font-medium transition-colors ${isOpen ? "text-white" : "text-[#1e1e1e] group-hover:text-[#ff5a14]"
                      }`}
                  >
                    {item.question}
                  </h3>
                  <svg
                    className={`h-6 w-6 transition-transform ${isOpen ? "text-white rotate-180" : "text-[#4a4a4a] group-hover:text-[#ff5a14]"
                      }`}
                    fill="none"
                    stroke="currentColor"
                    strokeWidth="2"
                ...
                {isOpen ? (
                  <div className="px-6 pb-6">
                    <p className="max-w-[840px] text-[16px] md:text-[18px] leading-relaxed text-white/95">
                      {item.answer}
                    </p>
                  </div>
                ) : null}
        ...
        <div className="mx-auto w-full max-w-[1220px] overflow-hidden rounded-[20px]">
          <div
          ...
          >
            ...
            <p className="mx-auto mt-3 max-w-[780px] text-[24px] text-[#4c4c4c]">
